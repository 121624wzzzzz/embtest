...
| tied / untied 实测 | `../extracts/*.info.json` 的 `tie_word_embeddings` 与 `cross_model_geometry` 结果列 `actual_tied_*` |
| GCorr Task1-4 结果 | [`../cross_model_geometry/results/`](../cross_model_geometry/results/) |
...
当前 `cross_model_geometry/results/task1-4` 结果表对应的 GCorr universe：
