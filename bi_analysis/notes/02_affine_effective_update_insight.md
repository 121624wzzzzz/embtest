...
> **10/13 vs 11/13 口径说明**：严格按数值 `1−R²_identity` 大小判，11/13 对满足 U > E；表中「10/13」按 **比值四舍五入到小数点后一位** 的口径（即 |(1−U)/(1−E) − 1| ≥ 0.05 才算有意义差异），将 Qwen3-30B-A3B（比值 1.047，四舍五入 1.0）归入「几乎相同」。本质上 10 和 11 的差异仅来自该对的四舍五入阈值，结论不变：仅 DeepSeek-V3/V3.1 是明确反例（U 变化反而更小），其余均为 U ≥ E。
