...
读取 extracts/ 中的 embedding 与 lm_head，输出到 results/。
"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="E/U checkpoint 几何审计")
    p.add_argument(
        "--scope",
    ...
    p.add_argument(
        "--svd-only",
        action="store_true",
        help="GPU Gram economy SVD 谱分析（PR/d、rank1、cos(v1,μ) 等）",
    )
    p.add_argument(
        "--device",
        default="cuda:0",
    ...
    if args.scope == "all" or args.merge_only:
        ...
        print(f"\nMerged 94-model tables -> {out_csv}", flush=True)
