...
| 项目 | 值 |
|------|-----|
| 配置 | `ijcai_clean/configs/base_instruct_pairs.yaml` |
...
对 31 行 `summary.csv` 聚合 `*_mean` 列：

| 指标 | 均值 | 中位数 | 最小 | 最大 |
|------|------|--------|------|------|
| `gcorr_E_cos_mean` | 0.970 | **0.998** | 0.777 | 1.000 |
| `gcorr_E_euc_mean` | 0.924 | 0.998 | 0.441 | 1.000 |
| `gcorr_U_cos_mean` | 0.970 | 0.998 | 0.777 | 1.000 |
...
**证据**：由 `model_a` 前缀映射 `series` 后对 `gcorr_E_cos_mean` 分组求均值（见 `data/computed_stats.json` → `task1.by_series_cos`）。

## E vs U：tied 与 untied

- **9/31** pair 的 `actual_tied_a` 或 `actual_tied_b` 为 `False`（典型：Qwen3-8B/14B Base↔Instruct、Qwen2.5 大模型、Llama-3.1）。
- 全体 E-U cos 差值 `gcorr_E_cos_mean - gcorr_U_cos_mean` 的均值 ≈ **-0.0003**（几乎对称）；untied 对上 U 可略高于或低于 E，无统一“IT 只改 U”模式。
...
- 31 对全部进入 Task6；`gcorr_E_cos_mean` 与 `E_R2` 相关 r≈0.93（见 [`../../affine/analysis/archive/laws_old.md`](../../affine/analysis/archive/laws_old.md) §2）。
- **主分析组**（去掉 Gemma-3-1B + Gemma-4 共 5 对）在 Task1 上 cos 中位数仍 ≈0.998，几乎不受尾部影响。
