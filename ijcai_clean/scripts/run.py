#!/usr/bin/env python3
"""ijcai_clean 数据驱动 CLI 派发入口。

用法:
  python ijcai_clean/scripts/run.py {task1|task2|task3|task4|task5|task6|list} [task 自身参数...]
  python ijcai_clean/scripts/run.py task1 --devices auto
  python ijcai_clean/scripts/run.py task5 --max-fit-rows 20000
  python ijcai_clean/scripts/run.py list
  python ijcai_clean/scripts/run.py task1 --help
...
def _task2(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=_REPO_ROOT / "ijcai_clean" / "results" / "task2_model_series",
    )
    add_gcorr_args(p)


def _run_task2(args: argparse.Namespace) -> None:
    from ijcai_clean.experiments.task2 import run_task2_model_series
...
def _task5(p: argparse.ArgumentParser) -> None:
    from ijcai_clean.experiments.task5_affine import (
...
def _run_task5(args: argparse.Namespace) -> None:
    from ijcai_clean.experiments.task5_affine import run_task5_affine_relations
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="run.py",
        description="ijcai_clean 任务派发入口；用 `run.py list` 查看可用任务。",
    )
