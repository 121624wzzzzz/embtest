...
| 路径 | 职责 |
|------|------|
| `configs/` | 任务配置：`base_instruct_pairs.yaml`（任务一）、`model_series.yaml`（任务二系列列表） |
| `src/ijcai_clean/` | **当前**分析代码包：加载 `extracts/`、对齐 tokenizer、GCorr、任务一/二汇总 |
| `scripts/` | 可执行入口，如 `run_task1_base_instruct.py`、`run_task2_model_series.py` |
| `results/` | 当前实验输出（如 `task1_base_instruct/`、`task2_model_series/`） |
| `legacy/` | **旧参考**：原 `run_exp1_v4.py`、仿射实验、矩阵对比脚本（路径与环境仍为历史口径） |
| `docs/` | 文档与说明 |

## 当前推荐入口（基于 extracts）

仓库根目录有 [`models.yaml`](/home/wz/projects/mypro/get_useful/models.yaml)（下载与模型简称映射）、[`extracts/`](/home/wz/projects/mypro/get_useful/extracts/)（按模型简称命名的 `*.safetensors` + `*.info.json`）。
...
**下载与抽取权重**：仓库根目录仍可使用 `python get_model_useful.py`（转发至 [`tools/download/get_model_useful.py`](../tools/download/get_model_useful.py)）。
