...
本文档记录当前 `cross_model_geometry` 主线实验的方法、数据接口、指标定义与运行口径。研究性结论和异常解释放在 `../analysis.md`，历史实验摘要放在 `historical/`。
...
- 2026-05-22：同步主线入口为 `cross_model_geometry/scripts/run.py task1..task6`，旧 `run_task*.py` wrapper 仅作为兼容入口保留。
...
| 任务 | 配置 | 入口 | 主要输出 |
|---|---|---|---|
| Task1 Base/Instruct GCorr | `configs/base_instruct_pairs.yaml` | `cross_model_geometry/scripts/run.py task1` | `cross_model_geometry/results/task1_base_instruct/summary.csv` |
| Task2 Model-Series GCorr | `configs/model_series.yaml` | `cross_model_geometry/scripts/run.py task2` | `cross_model_geometry/results/task2_model_series/summary.csv` |
| Task3 Cross-Scale GCorr | `configs/cross_scale_groups.yaml` + `configs/model_series.yaml` | `cross_model_geometry/scripts/run.py task3` | `cross_model_geometry/results/task3_cross_scale_groups/summary.csv` |
| Task4 MoE Cross-Family GCorr | `configs/moe_cross_family.yaml` + `configs/model_series.yaml` | `cross_model_geometry/scripts/run.py task4` | `cross_model_geometry/results/task4_moe_cross_family/summary.csv` |
| Task5 Affine Relations (子采样) | `configs/affine_pairs.yaml` | `cross_model_geometry/scripts/run.py task5` | `cross_model_geometry/results/task5_affine_subsampled/summary_pair.csv` |
| Task6 Base-Instruct full-vocab affine / SVD | `configs/base_instruct_pairs.yaml` | `cross_model_geometry/scripts/run.py task6` | `cross_model_geometry/results/task6_base_instruct_full_vocab/summary_pair_base_instruct_full_vocab.csv` |
...
推荐统一使用 `PYTHONPATH=cross_model_geometry/src python cross_model_geometry/scripts/run.py <task>` 运行主线任务。`run_task*_*.py`（含 Task6 的 `run_task6_base_instruct_full_vocab_affine.py`）仍保留为旧命令兼容 wrapper。
...
| 入口 | `PYTHONPATH=cross_model_geometry/src python cross_model_geometry/scripts/run.py task1 --devices auto`。 |
| 输出目录 | `cross_model_geometry/results/task1_base_instruct/`。 |
...
| 默认采样 | 与 Task1 相同。 |
| 入口 | `PYTHONPATH=cross_model_geometry/src python cross_model_geometry/scripts/run.py task2 --devices auto`。 |
| 输出目录 | `cross_model_geometry/results/task2_model_series/`。 |
...
| 核心方法 | 与 Task1 相同的 GCorr runner。 |
| 默认采样 | 与 Task1 相同。 |
| 入口 | `PYTHONPATH=cross_model_geometry/src python cross_model_geometry/scripts/run.py task3 --devices auto`。 |
| 输出目录 | `cross_model_geometry/results/task3_cross_scale_groups/`。 |
...
| 核心方法 | 与 Task1 相同的 GCorr runner。 |
| 默认采样 | 与 Task1 相同。 |
| 入口 | `PYTHONPATH=cross_model_geometry/src python cross_model_geometry/scripts/run.py task4 --devices auto`。 |
| 输出目录 | `cross_model_geometry/results/task4_moe_cross_family/`。 |
...
| 入口 | `PYTHONPATH=cross_model_geometry/src python cross_model_geometry/scripts/run.py task5 --devices auto`。 |
| 输出目录 | `cross_model_geometry/results/task5_affine_subsampled/`。 |
...
| 入口 | `PYTHONPATH=cross_model_geometry/src python cross_model_geometry/scripts/run.py task6`。 |
| 输出目录 | `cross_model_geometry/results/task6_base_instruct_full_vocab/`。 |
...
读取逻辑位于 `cross_model_geometry/src/cross_model_geometry/data.py`。
...
相关逻辑位于 `cross_model_geometry/src/cross_model_geometry/alignment.py`；full-vocab 诊断核心实现位于 `cross_model_geometry/src/cross_model_geometry/experiments/full_vocab_affine.py`，CLI wrapper 位于 `cross_model_geometry/scripts/run_task6_base_instruct_full_vocab_affine.py`。
...
这样可以避免显式构造大规模差分张量，提高 GPU 计算效率。实现位于 `cross_model_geometry/src/cross_model_geometry/metrics.py`。
...
- `actual_tied`：实际矩阵比较结果，即 `np.allclose(E, U, rtol=1e-5, atol=1e-5)`；当前主线代码位于 `cross_model_geometry/src/cross_model_geometry/data.py`。
