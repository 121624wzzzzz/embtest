...
- 数据复用：直接读各 task 的 ``generated_pairs.yaml``，token 对齐用
  ``cross_model_geometry.alignment.build_pair_token_info``，加载用 ``data._load_model_bundle``。
...
from cross_model_geometry.experiments.gcorr_io import load_pairs_yaml, try_git_commit
